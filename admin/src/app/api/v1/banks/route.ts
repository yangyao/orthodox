...
export async function GET(request: NextRequest) {
  ...
  const isRecommended = request.nextUrl.searchParams.get("isRecommended") === "true";

  const conditions = [eq(questionBanks.status, "published")];
  if (categoryId) conditions.push(eq(questionBanks.categoryId, BigInt(categoryId)));
  if (isRecommended) conditions.push(eq(questionBanks.isRecommended, true));
